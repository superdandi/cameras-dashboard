...
function fibonacciSphere(n: number, radius: number): [x: number, y: number, z: number][] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    points.push([
      ...
      y * radius * 0.7, // flatten Y for cinematic feel
      Math.sin(theta) * r * radius - 2, // shift back slightly
    ]);
  }
  return points;
}
...
// ─── Scene init ──────────────────────────────────────────────────────
export async function initSphereMatrix(
  container: HTMLElement,
  cameras: Camera[],
): Promise<SphereMatrix> {
  // ─── Renderer ────────────────────────────────────────────────────
  ...
  // ─── Spheres ─────────────────────────────────────────────────────
  const positions = fibonacciSphere(cameras.length, LAYOUT_RADIUS);
  ...
  for (let i = 0; i < cameras.length; i++) {
    ...
    // Load real texture async, update material when ready
    const tex = loadCameraTexture(cameras[i], (realTex) => {
      const old = mat.map;
      mat.map = realTex;
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    if (newHovered !== hoveredIndex) {
      ...
      spheres.forEach((s, i) => {
        ...
        mat.emissiveIntensity = i === hoveredIndex ? 0.4 : 0.1;
      });
  ...
  const refreshTextures = () => {
    for (let i = 0; i < cameras.length; i++) {
      const url = snapshotUrl(cameras[i].id);
      textureLoader.load(
        url,
        (tex) => {
          tex.colorSpace = THREE.SRGBColorSpace;
  ...
  const animate = () => {
    ...
    // Subtle floating animation for spheres
    for (let i = 0; i < spheres.length; i++) {
      const s = spheres[i];
      const offset = i * 0.5;
      s.position.y = positions[i][1] + Math.sin(now * 0.001 + offset) * 0.15;
    }

    // Subtle camera sway
    camera.position.x += Math.sin(now * 0.0003) * 0.002;
    camera.position.y += Math.cos(now * 0.0002) * 0.001;
  ...
  return {
    ...
    zoomToSphere: (index: number) => {
      ...
      camPos.z += 3; // camera 3 units in front of sphere
      return animateCamera(camPos, target.position, 1.0);
    },

    resetCamera: () => {
      return animateCamera(
        new THREE.Vector3(7, 5, 9),
        centerOfSpheres,
        0.8,
      );
    },
